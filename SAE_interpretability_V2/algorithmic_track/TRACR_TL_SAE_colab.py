"""
TRACR + TransformerLens + SAE Training Script (Google Colab Version)

Instructions:
1. Upload this file to Google Colab
2. Run Cell 0 and Cell 1 to install dependencies
3. RESTART THE RUNTIME (Runtime -> Restart runtime)
4. Skip Cell 0 and Cell 1, run from Cell 2 onwards

Note: The runtime restart is essential for the new package versions to take effect.
"""

# Cell 0
# Install compatible versions to avoid dependency conflicts

# First, uninstall pre-installed conflicting packages
%pip uninstall -y -q jax jaxlib

# Install torch 2.5.1 (highest available on cu121)
%pip install -q "torch==2.5.1" --index-url https://download.pytorch.org/whl/cu121

# Install NumPy 1.26.4 (compatible with both JAX 0.4.28 and transformer_lens)
%pip install -q "numpy==1.26.4"

# Install JAX 0.4.28 with compatible versions
%pip install -q jax==0.4.28 jaxlib==0.4.28 ml-dtypes==0.2.0 opt_einsum==3.3.0

# Install other dependencies with compatible versions
%pip install -q dm-haiku absl-py chex==0.1.86 optax==0.2.2 toolz==0.12.0 einops tabulate jmp networkx typing_extensions>=4.9.0
...
# Cell 1
# Install transformer_lens and tracr

import google.colab
IN_COLAB = True
print("Running as a Colab notebook")

# Install transformer_lens (version compatible with torch 2.5.1)
# Note: transformer_lens 2.15.0 and earlier work with torch 2.5.x
%pip install -q "transformer_lens<2.16"

# Install tracr
try:
    %pip install -q git+https://github.com/neelnanda-io/Tracr.git@main#egg=tracr
except:
    print("First attempt failed, trying alternative...")
    %pip install -q git+https://github.com/neelnanda-io/Tracr.git

print("\n✅ Installation complete!")
print("⚠️ IMPORTANT: You must RESTART THE RUNTIME now for changes to take effect!")
print("After restarting, skip Cell 0 and Cell 1, and run from Cell 2 onwards.")
